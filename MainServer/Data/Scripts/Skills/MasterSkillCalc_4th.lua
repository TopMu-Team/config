...
function WindSoul_4thEnchant_Lord(InDamage, Strength, Dexterity, Vitality, Energy, Command)
	local OutDamage = InDamage * (Strength / 20) / 100
...
function MagicPin_4thEnchant_Lancer(InDamage, Dexterity, SkillTreeBonus, BarrageCount)
	...
	if (BarrageCount == 1) then
		OutDamage = (InDamage * 0.8) * (Dexterity / 18 + 97 + SkillTreeBonus) / 100.0
	elseif (BarrageCount == 2) then
		OutDamage = (InDamage * 1.0) * (Dexterity / 18 + 97 + SkillTreeBonus) / 100.0
	elseif (BarrageCount == 3) then
		OutDamage = (InDamage * 1.1) * (Dexterity / 18 + 97 + SkillTreeBonus) / 100.0
	elseif (BarrageCount == 4) then
		OutDamage = (InDamage * 1.2) * (Dexterity / 18 + 97 + SkillTreeBonus) / 100.0
	elseif (BarrageCount == 5) then
		OutDamage = (InDamage * 1.3) * (Dexterity / 18 + 97 + SkillTreeBonus) / 100.0
	end
...
function DeadScythe_4thEnchant_Summoner(InDamage, TargetClass, Strength, Dexterity, Vitality, Energy, BarrageCount)
	...
	if (BarrageCount == 1) then
		OutDamage = (InDamage * 0.8) * ( 200 + ( Energy / 30 ) ) / 100
	elseif (BarrageCount == 2) then
		OutDamage = (InDamage * 1.0) * ( 200 + ( Energy / 30 ) ) / 100
	elseif (BarrageCount == 3) then
		OutDamage = (InDamage * 1.1) * ( 200 + ( Energy / 30 ) ) / 100
	elseif (BarrageCount == 4) then
		OutDamage = (InDamage * 1.2) * ( 200 + ( Energy / 30 ) ) / 100
	elseif (BarrageCount == 5) then
		OutDamage = (InDamage * 1.3) * ( 200 + ( Energy / 30 ) ) / 100
	end
...
function BatFlock_DotDamage_4thEnchant_Slayer(InDamage, Strength, Energy)
	local Damage = (InDamage * 1.0) * (( Strength / 35 ) ) / 100
	local DotDamage = (InDamage / 20)
	local Time = 3
...
function BatFlock_4thEnchant_Slayer(InDamage, Strength, Dexterity)
	local OutDamage = InDamage * ((Strength / 25) + (Dexterity / 25)) / 100
...
function PierceAttack_4thEnchant_Slayer(InDamage, Strength, Dexterity)
	local OutDamage = InDamage * ((Strength / 20) + (Dexterity / 20)) / 100
	return OutDamage
end
...
function SolidProtection_4thEnchant_Knight(NormalLevel, MasterLevel, Strength, Dexterity, Vitality, Energy)
	local AbsorbHP = Energy / 1200
	local ConvertDamage = Energy / 1200
	local IncAtkPower = Energy / 1200
	local Duration = 180
...
function IceBlast_4thEnchant_GunCrusher(InDamage, Strength, Dexterity, Vitality, Energy)
	local OutDamage = 0
	OutDamage = InDamage * ((Energy / 22) + (Dexterity / 28) + 120) / 100
...
function BurstingFlare_4thEnchant_GunCrusher(InDamage, Strength, Dexterity, Vitality, Energy, BarrageCount)
	...
	elseif (BarrageCount == 3) then -- Shockwave
		OutDamage = (InDamage * 1.3) * ((Energy / 20)) / 100
	end
...
function LightWizardSpearStorm_4thEnchant_Calc(InDamage, Strength, Dexterity, Vitality, Energy, BarrageCount)
	...
	if (BarrageCount == 1) then
		OutDamage = ((InDamage * 1.0) * ((Energy / 110))) / 100
	elseif (BarrageCount == 2) then
		OutDamage = ((InDamage * 1.0) * ((Energy / 110))) / 100
	elseif (BarrageCount == 3) then
		OutDamage = ((InDamage * 1.0) * ((Energy / 110))) / 100
...
function LightWizardDragonViolent_4thEnchant_Calc(InDamage, Strength, Dexterity, Vitality, Energy, BarrageCount)
	...
	if (BarrageCount == 1) then
		OutDamage = ((InDamage * 1.0) * ((Energy / 75) )) / 100
	elseif (BarrageCount == 2) then
		OutDamage = ((InDamage * 1.0) * ((Energy / 75) )) / 100
	elseif (BarrageCount == 3) then
		OutDamage = ((InDamage * 1.0) * ((Energy / 75) )) / 100
...
function ElfRainingArrow_4thEnchant_Calc(InDamage, Strength, Dexterity, Vitality, Energy, BarrageCount)
	...
	if (BarrageCount == 1) then
        OutDamage = ((InDamage * 1.1) * (200 + (Dexterity / 20))) / 100
    elseif (BarrageCount == 2) then
        OutDamage = ((InDamage * 1.15) * (200 + (Dexterity / 20))) / 100
    elseif (BarrageCount == 3) then
        OutDamage = ((InDamage * 1.2) * (200 + (Dexterity / 20))) / 100
	elseif (BarrageCount == 4) then
        OutDamage = ((InDamage * 1.25) * (200 + (Dexterity / 20))) / 100
	elseif (BarrageCount == 5) then
        OutDamage = ((InDamage * 1.3) * (200 + (Dexterity / 20))) / 100
    end
...
function IllusionKnightWindGlaive_4thEnchant_Calc(InDamage, Strength, Dexterity, Vitality, Energy, BarrageCount)
	...
	if (BarrageCount == 1) then
		OutDamage = (InDamage * ((Strength / 15) + (Dexterity / 15))) / 100
	elseif (BarrageCount == 2) then
		OutDamage = (InDamage * ( (Strength / 15) + (Dexterity / 15))) / 100
	elseif (BarrageCount == 3) then
		OutDamage = (InDamage * ((Strength / 15) + (Dexterity / 15))) / 100
